...
export default async function handler(request: any, response: any) {
  try {
    ...
    const rawUrl = String(request.url || '/api/instructor/me');
    ...
    console.error('Instructor me Vercel wrapper failed:', error);
    response.statusCode = 500;
    response.setHeader('content-type', 'application/json');
    response.end(JSON.stringify({ ok: false, error: 'Instructor API server error' }));
  }
}
