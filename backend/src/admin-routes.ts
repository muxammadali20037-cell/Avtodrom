...
async function requireAdmin(request: any, authenticate: (request: any) => Promise<TelegramWebAppUser>) {
  ...
  if (!profile && configuredAdminId && String(user.id) === configuredAdminId) {
    const created = await supabaseRest<any[]>('profiles', {
      method: 'POST', headers: { Prefer: 'return=representation' },
      body: JSON.stringify({ telegram_id: user.id, first_name: user.first_name || null, last_name: user.last_name || null, username: user.username || null, role: 'admin' }),
    });
  ...
  if (profile.role !== 'admin' && String(user.id) === configuredAdminId) {
    const promoted = await supabaseRest<any[]>('profiles', { method: 'PATCH', headers: { Prefer: 'return=representation' }, query: `?id=eq.${q(profile.id)}`, body: JSON.stringify({ role: 'admin', updated_at: new Date().toISOString() }) });
    profile = promoted[0] || profile;
  }
  return { user, profile };
}
...
export async function registerAdminRoutes(app: FastifyInstance, authenticate: (request: any) => Promise<TelegramWebAppUser>) {
  ...
  app.get('/api/admin/instructors', async (request, reply) => {
    try { await requireAdmin(request, authenticate); return { ok:true, instructors:await supabaseRest<any[]>('instructors',{query:'?select=id,active,created_at,profile:profile_id(id,telegram_id,first_name,last_name,username,phone,role)&order=created_at.desc'}) }; }
  ...
  app.patch<{Params:{id:string};Body:{active?:boolean}}>('/api/admin/instructors/:id',async(request,reply)=>{
    try{await requireAdmin(request,authenticate);const rows=await supabaseRest<any[]>('instructors',{method:'PATCH',headers:{Prefer:'return=representation'},query:`?id=eq.${q(request.params.id)}`,body:JSON.stringify({active:Boolean(request.body?.active)})});if(!rows[0])return reply.code(404).send({ok:false,error:'Instructor topilmadi'});return{ok:true,instructor:rows[0]};}
  ...
  app.get('/api/admin/bookings',async(request,reply)=>{
    try{await requireAdmin(request,authenticate);const query=request.query as {status?:string};const parts=['select=*,customer:customer_id(id,first_name,last_name,username,phone),instructor:instructor_id(id,profile:profile_id(first_name,last_name,username)),car:car_id(id,plate_number,model)','order=start_at.desc'];if(query.status)parts.push(`status=eq.${q(query.status)}`);return{ok:true,bookings:await supabaseRest<any[]>('bookings',{query:`?${parts.join('&')}`})};}
    catch(e){return reply.code(403).send({ok:false,error:e instanceof Error?e.message:'Failed to load bookings'});}
  });

  app.get('/api/admin/customers',async(request,reply)=>{try{await requireAdmin(request,authenticate);return{ok:true,customers:await supabaseRest<any[]>('profiles',{query:'?role=eq.customer&select=id,telegram_id,first_name,last_name,username,phone,created_at&order=created_at.desc'})};}catch(e){return reply.code(403).send({ok:false,error:e instanceof Error?e.message:'Failed to load customers'});}});
  app.get('/api/admin/cars',async(request,reply)=>{try{await requireAdmin(request,authenticate);return{ok:true,cars:await supabaseRest<any[]>('cars',{query:'?select=*&order=created_at.desc'})};}catch(e){return reply.code(403).send({ok:false,error:e instanceof Error?e.message:'Failed to load cars'});}});
  app.post('/api/admin/cars',async(request,reply)=>{try{await requireAdmin(request,authenticate);const body=(request.body||{}) as {plate_number?:string;model?:string};const plate=String(body.plate_number||'').trim().toUpperCase();const model=String(body.model||'').trim();if(!plate||!model)return reply.code(400).send({ok:false,error:'Raqam va rusum majburiy'});const rows=await supabaseRest<any[]>('cars',{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify({plate_number:plate,model,active:true})});return reply.code(201).send({ok:true,car:rows[0]});}catch(e){return reply.code(400).send({ok:false,error:e instanceof Error?e.message:'Car creation failed'});}});
}
