...
async function notifyInstructorDecision(
  ...
  app.post('/api/admin/courses', async (req: any, reply: any) => {
    try {
      ...
      /* Kategoriya PATCH da saqlanardi-yu, YARATISHDA tushib qolgan edi:
         yangi mashg'ulot kategoriyasiz yozilar va kassada tanlov
         ro'yxatiga chiqmasdi. */
      const cat = String(b.category || '').trim().toUpperCase();
      if (!['A', 'B', 'C'].includes(cat)) {
        return reply.code(400).send({ ok: false, error: 'Kategoriya A, B yoki C bo‘lsin' });
      }
