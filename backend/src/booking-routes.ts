...
export async function registerBookingRoutes(app: FastifyInstance, authenticate: (request: any) => Promise<TelegramWebAppUser>) {
  app.get('/api/me', async (request, reply) => {
    try { return { ok: true, profile: await profileForTelegram(await authenticate(request)) }; }
    catch (e) { return reply.code(401).send({ ok: false, error: 'Unauthorized' }); }
  });
  ...
  app.get('/api/bookings', async (request, reply) => {
    try {
      ...
      if (role === 'customer') parts.push(`customer_id=eq.${q(profile.id)}`);
      if (role === 'instructor') {
        const ins = await supabaseRest<any[]>('instructors', { query: `?profile_id=eq.${q(profile.id)}&select=id` });
        if (!ins[0]) return { ok: true, bookings: [] };
        parts.push(`instructor_id=eq.${q(ins[0].id)}`);
      }
  ...
  app.post('/api/bookings', async (request, reply) => {
    try {
      const user = await authenticate(request);
      const profile = await profileForTelegram(user);
      const body = request.body as { instructor_id?: string; car_id?: string; start_at: string; end_at: string; customer_note?: string };
      ...
      if (!(start < end) || start < new Date()) return reply.code(400).send({ ok: false, error: 'Invalid booking time' });
      const conflicts = await supabaseRest<any[]>('bookings', { query: `?start_at=lt.${q(body.end_at)}&end_at=gt.${q(body.start_at)}&status=in.(pending,confirmed,in_progress)&select=id,instructor_id,car_id` });
      ...
      if (booking) {
        const targets = [profile.id];
        if (body.instructor_id) {
          const ins = await supabaseRest<any[]>('instructors', { query: `?id=eq.${q(body.instructor_id)}&select=profile_id` });
          if (ins[0]?.profile_id) targets.push(ins[0].profile_id);
        }
        await supabaseRest('notifications', { method: 'POST', body: JSON.stringify(targets.map(profile_id => ({ profile_id, booking_id: booking.id, title: 'Yangi bron', body: 'Yangi mashg‘ulot broni yaratildi.', channel: 'in_app' }))) });
      }
  ...
  app.patch('/api/bookings/:id/status', async (request, reply) => {
    try {
      const user = await authenticate(request);
      const profile = await profileForTelegram(user);
      if (!['admin','instructor'].includes(profile.role)) return reply.code(403).send({ ok: false, error: 'Forbidden' });
      ...
      const rows = await supabaseRest<any[]>('bookings', { method: 'PATCH', headers: { Prefer: 'return=representation' }, query: `?id=eq.${q((request.params as any).id)}`, body: JSON.stringify({ status: body.status, cancelled_reason: body.reason || null, updated_at: new Date().toISOString() }) });
      if (!rows[0]) return reply.code(404).send({ ok: false, error: 'Booking not found' });
      const booking = rows[0];
  ...
  app.get('/api/notifications', async (request, reply) => {
    try {
      ...
      return { ok: true, notifications: await supabaseRest<any[]>('notifications', { query: `?profile_id=eq.${q(profile.id)}&select=*&order=created_at.desc&limit=100` }) };
    } catch (e) { return reply.code(401).send({ ok: false, error: 'Unauthorized' }); }
  });
}
