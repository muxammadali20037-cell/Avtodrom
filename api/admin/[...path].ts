...
export default async function handler(request: any, response: any) {
  try {
    ...
    const pathname = parsed.pathname.startsWith('/api/admin')
      ? parsed.pathname
      : `/api/admin${parsed.pathname.startsWith('/') ? parsed.pathname : `/${parsed.pathname}`}`;
    ...
    console.error('Vercel admin handler failed:', error);
