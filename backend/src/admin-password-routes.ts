...
function valid(t: string) {
  try {
    const secret = String(process.env.ADMIN_SESSION_SECRET || process.env.ADMIN_PASSWORD || '').trim();
    const a = Buffer.from(t || '', 'base64url').toString('utf8').split(':');
  ...
  app.get('/api/admin/stats', async (req: any, reply: any) => {
    try {
      ...
      const [users, instructors, bookings, reviews, payments] = await Promise.all([
        supabaseRest<any[]>('users', { query: '?select=id,role,is_active,is_blocked' }),
        supabaseRest<any[]>('instructor_profiles', { query: '?select=id,user_id,is_available,rating,total_reviews,is_verified' }),
