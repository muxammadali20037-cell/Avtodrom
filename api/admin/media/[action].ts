...
async function commit(body:any) {
  ...
  const patched=await rest('admin_media',{
    method:'PATCH',
    headers:{Prefer:'return=representation'},
    query:`?key=eq.${encodeURIComponent(mediaKey)}`,
    body:JSON.stringify({title,media_type:mediaType,storage_path:path,public_url:publicUrl,is_active:true,updated_at:new Date().toISOString()}),
  });
  let media=Array.isArray(patched) ? patched[0] : null;
  if(!media){
    const created=await rest('admin_media',{
      method:'POST',headers:{Prefer:'return=representation'},
      body:JSON.stringify({key:mediaKey,title,media_type:mediaType,storage_path:path,public_url:publicUrl,is_active:true}),
    });
...
export default async function handler(request:any,response:any){
  try{
    ...
    response.statusCode=action==='commit'?201:200;
    ...
    response.end(JSON.stringify({ok:false,error:error instanceof Error?error.message:'Media operation failed'}));
  }
}
