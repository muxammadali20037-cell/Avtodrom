...
import { sendCustomerStart, startCustomerPolling, validateTelegramInitData } from './telegram.js';
...
app.post<{ Body: { initData?: string } }>('/api/telegram/auth', async (request, reply) => {
  try {
    ...
    const user = validateTelegramInitData(initData, CUSTOMER_BOT_TOKEN);
    return { ok: true, user };
  } catch {
