...
async function authenticate(request: any): Promise<TelegramWebAppUser> { const initData = String(request.headers['x-telegram-init-data'] || '').trim(); if (!initData) throw new Error('Telegram initData missing'); return validateTelegramInitData(initData, CUSTOMER_BOT_TOKEN); }
...
if (CUSTOMER_BOT_TOKEN && MINI_APP_URL) void startCustomerPolling(CUSTOMER_BOT_TOKEN, MINI_APP_URL); else app.log.warn('CUSTOMER_BOT_TOKEN or MINI_APP_URL is missing; Telegram polling is disabled.');
